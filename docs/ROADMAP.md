...
| 4 | Minimal rootfs | Buildroot initramfs with `wana-init` (Rust) as PID 1; UEFI boot reaches `[INIT] info: ready` and powers off | IN PROGRESS | [phase 04 report](test-reports/phase-04.md) |
| 5 | UEFI boot with bootloader | GRUB (x86_64-efi) on an EFI System Partition boots kernel + rootfs in QEMU+OVMF (no `-kernel` shortcut) | NOT STARTED | |
