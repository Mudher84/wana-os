//! SHA-256 (FIPS 180-4), for checking font files against `SHA256SUMS`.
//! Small and dependency-free; checked against the standard test vectors.
...
/// Lowercase hex, as `sha256sum` prints it.
...
mod tests {
    ...
    fn standard_test_vectors() {
        ...
        for (input, want) in cases {
            assert_eq!(hex(&digest(input)), want, "{input:?}");
        }
    }

    #[test]
    fn padding_boundaries() {
        // 55, 56 and 64 bytes switch between one and two padding blocks.
        let want = [
            (
                55,
        ...
        for (n, h) in want {
            assert_eq!(hex(&digest(&vec![b'a'; n])), h, "{n} bytes");
        }
    }
}
